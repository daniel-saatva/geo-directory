...
namespace :users do
  ...
  task import_csv: :environment do
    ...
    csv.each do |row|
      ...
      user = User.new(row.to_hash.except('squad','lat','lng','country','city','start_date','birth_date'))
      ...
      user.birth_date = Date.strptime(row['birth_date'], '%m/%d/%y') if row['birth_date'].present?
      user.save
    end
  end
end
